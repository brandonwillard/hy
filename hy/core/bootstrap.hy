...
(defmacro macro-error [reason expression &optional [filename '--name--]]
...
(defmacro defn [name lambda-list &rest body]
  ...
  (if (not (= (type name) hy.HySymbol))
    (macro-error "defn takes a name as first argument" name))
  (if (not (isinstance lambda-list hy.HyList))
    (macro-error "defn takes a parameter list as second argument" name))
...
(defmacro defn/a [name lambda-list &rest body]
  ...
  (import hy)
  (if (not (= (type name) hy.HySymbol))
    (macro-error "defn/a takes a name as first argument" name))
  (if (not (isinstance lambda-list hy.HyList))
    (macro-error "defn/a takes a parameter list as second argument" name))
